...
cor$r #лучше всего коррелируют между собой math и read
cor$p #соответственно уровень p.value (значимый эффект взаимосвязи)
...
summary(fit) 
#вызвав информацию относительно нашей модели, мы видим,что все независимые переменные имеют значимый эффект (p.val<0.05)
...
#рассчитывает true positive rate и false positive rate:
...
str(auc) #площадь 0.87 - очень неплохое значение
...
#по графику видно, что все три кривые перескаются в одной точке. Значение порога вероятности в этой точке = 0.225 - это и будет оптимальное значение 
...
#процент правильно предсказанных значений равен 76.7 (что также видно на ROC кривой для порога 0.225)
...
#процент правильно предсказанных значений равен 74